# BADGER GRAPPLE RED - THE VISION (v1.1 RATIFIED)

The founding document. Every manifesto, spec, system, and sprite hangs
under this. When any decision is unclear, settle it against §1.

## 1. The fantasy and the why

**You are a freshman walk-on at Wisconsin. You wake up in the Camp
Randall locker room on day one of the season; by spring you have built
the room around you and brought a national title home.**

This game is for the team and the recruits. Every decision serves two
feelings: "I know this place" (Madison, rendered with love — recognition
is the product) and "this is what a season feels like" (Pokémon's
comfort-loop carrying wrestling's soul). We do not reskin Pokémon; we
translate its spatial and emotional grammar into wrestling's native
language (CITY_DESIGN_MANIFESTO Part X is the translation dictionary).

## 2. Who you are

A freshman wrestler - NOT the coach. Coach is the Professor figure. On day one
you pick your **persona** - your style (Shooter / Rider / Scrambler) and its animal
spirit — and *you are your own starter*: your persona is roster slot #1
and wrestles in your duals. Rex is a returning teammate in your weight class;
the opening wrestle-off and his transfer make him the rival who stays ahead all
season.

## 3. The cast (correspondence table)

| FireRed | Ours |
|---|---|
| Your bedroom | Your LOCKER, Camp Randall locker room: wake, save, return |
| Mom | The team CAPTAIN: sends you to Coach day one; home warmth |
| Professor Oak | COACH, in the Camp Randall wrestling room |
| Starter choice | Persona/style pick; you are your own starter |
| Rival | Rex, counter-style freshman |
| Pokedex | THE ROSTER BOOK: every wrestler in the region, seen/signed |
| Catching | Recruiting at open mats (BUILT) |
| Party of 6 | The TRAVEL LINEUP (6) |
| PC box | THE LOCKER stores the rest of the roster (NEW SYSTEM) |
| Center / Mart | Trainer's Room / Bucky's Locker Room franchise words (law 7b) |
| Gym + badge | Duals/tournaments at real venues; credentials ladder |
| HMs / map verbs | Film Study (reveal/unlock), Conditioning Cert (sleds), Bus Pass (return travel) |
| Elite Four | The NCAA bracket, St. Louis: quarters, semis, final |
| Hall of Fame | The trophy case's empty center space (BUILT, waiting) |
| Types | Six wrestling styles + asymmetric ADV chart (DESIGNED, code pending) |
| Evolution | Development ceremonies (BUILT) |
| Legendary | A hidden legend of the sport (Season Two seam) |

## 4. The world (Season One region)

CAMP RANDALL (home town: locker room, wrestling room, Coach, captain)
-> R1 -> FIELD HOUSE town (★1, where the team really competes; X: the
Field House + Arch) -> R2 = STATE STREET -> CAPITOL SQUARE (★2; X: the
enterable Capitol) -> R3 Monona shore -> KOHL CENTER town (★3 = the
conference tournament) -> AIRPORT -> flight -> ST. LOUIS (NATIONALS;
X: the Gateway Arch). BASCOM HILL optional north branch off R2; the
western wilds (Lakeshore Path / Picnic Point) unchanged, hanging off
the Field House town. Region map: the boarded proposal (Tony-reviewed).
Geometry law: one plane, validator-checked, Town Map generated from it.

## 5. The loop

- **Minute**: walk a route, cross open mats, scout, recruit, get
  ambushed, manage the lineup.
- **Town**: arrive -> recover -> shop -> find the X-factor -> earn the
  badge -> a gate you remember opens.
- **Season (the story)**: preseason in the locker room -> duals ->
  conference at the Kohl -> the flight -> St. Louis -> walk back into
  Camp Randall and read the trophy case. The first area matters last.
  The existing day/period clock is the season calendar.

## 6. Systems: built vs needed

BUILT: ratified six-style combat, Condition/Stamina, opponent AI,
scouting + three-tier Singlet recruiting, encounter slots + blue chips,
development, meaningful practice tracks, six-wrestler travel lineup,
Team Locker storage, Trainer's Room recovery, shop and Bag economy,
venue badges/gates, tournament bracket, Bus Pass travel, flight gate,
Roster Book seen/defeated/signed tracking, Town Map, one-plane law,
FireRed cadence, open mats, day/period clock, and save migration.
NEEDED: captain-at-home + locker-room opening sequence,
player-as-freshman fiction pass (dialogue, intro, naming), final map event
placement, encounter/team balance against the approved maps, and art polish.

## 7. Remaining open calls (Tony)

1. Kohl Center District's non-arena X-factor.
2. Minor NPC identities listed in `SEASON_ONE_SYNOPSIS.md`.
3. Monona Shore's optional side activity.

The player pivot, venue-based badge names, Bascom branch, and required Picnic
Point Badge are ratified in `SEASON_ONE_SYNOPSIS.md`. The build order follows
`WORLD_MAP_MANIFESTO.md` v2 and `src/data/seasonOneRegion.json`: Camp Randall,
then R1 and Field House Town, then the rest of the new region. We do not repaint
the obsolete world twice.
