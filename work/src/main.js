...
const config={type:Phaser.AUTO,parent:'game',backgroundColor:'#000',pixelArt:true,roundPixels:true,width:240,height:170,scale:{mode:Phaser.Scale.FIT,autoCenter:Phaser.Scale.CENTER_BOTH},render:{antialias:false,pixelArt:true,roundPixels:true},audio:{noAudio:true},scene:[BootScene,TitleScene,IntroScene,StarterScene,OverworldScene,ScoutScene,BattleScene,MenuScene]};
let game;try{game=new Phaser.Game(config);window.badgerGame=game;window.BADGER_VERSION='21.3-shop-recovery-rooms';}catch(e){fail(e?.stack||e?.message||String(e));throw e;}
