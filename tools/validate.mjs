...
if(!existsSync(worldTilesetBuildPath))errs.push('Season One world tileset is missing; run npm run build:world-tileset');
  ...
  if(!groundMetrics.mowedGrass||groundMetrics.mowedGrass.uniqueColors>3||groundMetrics.mowedGrass.meanLightness<0.74||groundMetrics.mowedGrass.meanSaturation>0.43||groundMetrics.mowedGrass.cardinalPixelCount!==0)errs.push('Season One maintained lawn violates the quiet-ground contract');
  if(!groundMetrics.campusPavers||groundMetrics.campusPavers.uniqueColors>3||groundMetrics.campusPavers.meanLightness<0.86||groundMetrics.campusPavers.meanSaturation>0.40||groundMetrics.campusPavers.cardinalPixelCount!==0)errs.push('Season One campus pavers violate the pale neutral-limestone contract');
  if(!groundMetrics.primaryStonePath||groundMetrics.primaryStonePath.uniqueColors!==3||groundMetrics.primaryStonePath.meanLightness<0.82||groundMetrics.primaryStonePath.meanLightness>0.88||groundMetrics.primaryStonePath.meanSaturation>0.16||groundMetrics.primaryStonePath.accentPixelCount<28||groundMetrics.primaryStonePath.accentPixelCount>56||groundMetrics.primaryStonePath.meanNeighborContrast<0.01||groundMetrics.primaryStonePath.meanNeighborContrast>0.06||groundMetrics.primaryStonePath.cardinalPixelCount!==0)errs.push('Season One primary stone path violates the legible three-shade circulation contract');
  if(groundValueContract.grass?.meanLightnessMin!==0.78||groundValueContract.grass?.meanSaturationMin!==0.44||groundValueContract.grass?.meanSaturationMax!==0.49||groundValueContract.mowedGrass?.meanLightnessMin!==0.74||groundValueContract.mowedGrass?.meanSaturationMax!==0.43||groundValueContract.campusPavers?.meanLightnessMin!==0.86||groundValueContract.primaryStonePath?.uniqueColorsMin!==3||groundValueContract.primaryStonePath?.meanLightnessMin!==0.82||groundValueContract.primaryStonePath?.meanLightnessMax!==0.88||groundValueContract.primaryStonePath?.meanSaturationMax!==0.16||groundValueContract.primaryStonePath?.meanNeighborContrastMin!==0.01||groundValueContract.primaryStonePath?.meanNeighborContrastMax!==0.06)errs.push('Season One ground value thresholds are missing or stale');
...
if(!existsSync(campMetatileBuildPath))errs.push('Camp metatile build is missing; run npm run build:camp-metatiles');
  ...
  if(!metatileBuild.visualHierarchyMetrics||metatileBuild.visualHierarchyMetrics.contrastDifference<=0.04||metatileBuild.visualHierarchyMetrics.ground?.meanNeighborContrast>=metatileBuild.visualHierarchyMetrics.identityObjects?.meanNeighborContrast||metatileBuild.visualHierarchyMetrics.ground?.highContrastEdgeDensity>=metatileBuild.visualHierarchyMetrics.identityObjects?.highContrastEdgeDensity)errs.push('Camp Randall ground is not spatially quieter than its identity architecture');
