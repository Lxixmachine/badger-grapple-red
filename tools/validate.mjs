...
if(!existsSync(worldTilesetBuildPath))errs.push('Season One world tileset is missing; run npm run build:world-tileset');
  ...
  if(!groundMetrics.primaryStonePath||groundMetrics.primaryStonePath.uniqueColors>2||groundMetrics.primaryStonePath.meanLightness<0.88||groundMetrics.primaryStonePath.meanSaturation>0.16||groundMetrics.primaryStonePath.accentPixelCount>40||groundMetrics.primaryStonePath.cardinalPixelCount!==0)errs.push('Season One primary stone path violates the cool low-noise circulation contract');
  if(groundValueContract.grass?.meanLightnessMin!==0.78||groundValueContract.grass?.meanSaturationMin!==0.44||groundValueContract.grass?.meanSaturationMax!==0.49||groundValueContract.mowedGrass?.meanLightnessMin!==0.74||groundValueContract.mowedGrass?.meanSaturationMax!==0.43||groundValueContract.campusPavers?.meanLightnessMin!==0.86||groundValueContract.primaryStonePath?.meanLightnessMin!==0.88||groundValueContract.primaryStonePath?.meanSaturationMax!==0.16)errs.push('Season One ground value thresholds are missing or stale');
