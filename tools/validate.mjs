...
if(!existsSync(worldTilesetBuildPath))errs.push('Season One world tileset is missing; run npm run build:world-tileset');
  ...
  if(!groundMetrics.grass||groundMetrics.grass.uniqueColors!==2||groundMetrics.grass.dominantCoverage<0.94||groundMetrics.grass.accentPixelCount!==12||groundMetrics.grass.accentComponentCount!==4||groundMetrics.grass.meanLightness<0.76||groundMetrics.grass.meanLightness>0.80||groundMetrics.grass.meanSaturation>0.42||groundMetrics.grass.cardinalPixelCount!==0)errs.push('Season One grass violates the high-key connected-motif ground contract');
  ...
  if(!groundMetrics.mowedGrass||groundMetrics.mowedGrass.uniqueColors>3||groundMetrics.mowedGrass.meanLightness<0.68||groundMetrics.mowedGrass.meanSaturation>0.42||groundMetrics.mowedGrass.cardinalPixelCount!==0)errs.push('Season One maintained lawn violates the quiet-ground contract');
  ...
  if(groundValueContract.grass?.meanLightnessMin!==0.76||groundValueContract.mowedGrass?.meanLightnessMin!==0.68||groundValueContract.campusPavers?.meanSaturationMax!==0.40)errs.push('Season One ground value thresholds are missing or stale');
