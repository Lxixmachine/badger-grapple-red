...
test('production build boots the Season One atlas and generated character art', async ({page}) => {
  ...
  await expect.poll(async () => page.evaluate(() => window.BADGER_VERSION)).toBe('22.41-technique-choreography');
...
test('opening persona choice gives Rex the counter style and launches the wrestle-off', async ({page}) => {
  ...
  await expect.poll(async () => page.evaluate(() => window.__badgerTest.sceneState('BattleScene')), {timeout: 10_000}).toMatchObject({
    battleType: 'opening',
    trainerName: 'Rex',
    inputLocked: false,
    battlePhase: 'command'
