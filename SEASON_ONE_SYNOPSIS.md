# Badger Grapple Red: Season One — Full Step-by-Step Synopsis

> Built collaboratively with Tony, beat by beat, in the format of the
> Pokémon Red synopsis that inspired it. This is the story spine —
> GAME_VISION.md remains the founding "why," this is the "what happens."

## The Basic Premise

You are a freshman walk-on wrestler at Wisconsin, in Madison. On day one
of the season, your captain sends you to Coach, who has you pick your
wrestling persona — you are your own starter, wrestling in your own
duals all season. At the same time you take on the season's real
challenge:

1. Travel the Madison region.
2. Beat three captains at three campus venues.
3. Win the conference tournament at Kohl Center.
4. Fly to St. Louis for the NCAA Championships.
5. Beat the bracket.
...
# Act I: Waking Up at Camp Randall
...
Coach has you pick a style and its animal spirit:

- **Shooter** (Grizzly) — leg attacks, first-strike speed
- **Rider** (Gorilla) — mat control, tilts and cradles
- **Scrambler** (Red Panda) — transition wrestling, chaos as offense

Unlike Red, there is no separate "starter" sitting in a ball on a table
...
Poké Balls. R1 carries real density for a first route:

- An optional early Rex rematch.
- An NPC who teaches you the recruiting/singlet mechanic hands-on —
  this world's version of the old man's catching lesson.
- A denser trainer/mat gauntlet right before the Field House gate.
...
now become real recruiting attempts, spending a **Wisconsin Badgers
Singlet** —

- **Practice Singlet** (common)
- **Travel Quad Singlet** (better odds)
- **Starter Singlet** (best, rare)

— the same three-tier structure as Poké Ball / Great Ball / Master Ball.
...
Badge** (renamed from "Scramble Badge" for venue consistency) from
...
the ability to step away between them** to recover, shop, and manage
...
relentless pace before the true final: Rex, who reached the
championship match a step ahead of you one last time. Beating him ends
...
# Open calls still needing Tony's word

1. Minor NPC specifics not yet named: the R1 recruiting-lesson NPC, the
   Capitol booster/alum who gives the Kayak Voucher, the Brittingham
   Boats staffer, Nationals' round-one opponent.
2. R3's "fishing-style side activity" (mentioned when Monona was locked
...
# Implementation note — the style-system rework is NOT yet in code

This synopsis assumes a style rename and a new advantage chart that
Tony and Claude designed together this session but have not yet built:

- **Renames:** Neutral→Shooter, Top→Rider, Scramble→Scrambler,
  Pace→Bull, Upperbody→Thrower, Defense→Wall.
- **New asymmetric ADV chart** (replaces the old single 6-loop in
  `moves.js`), each style strong vs 2 / weak vs 2 / neutral vs 1:

  | Style | Strong vs | Weak vs |
  |---|---|---|
  | Shooter | Rider, Bull | Wall, Scrambler |
  | Scrambler | Shooter, Wall | Rider, Thrower |
  | Rider | Scrambler, Bull | Shooter, Thrower |
  | Bull | Wall, Thrower | Shooter, Rider |
  | Thrower | Rider, Scrambler | Wall, Bull |
  | Wall | Shooter, Thrower | Bull, Scrambler |

- Player still starts from exactly 3 personas (Shooter/Rider/Scrambler);
  restricted to just those three, the chart is still a clean triangle
  (Shooter>Rider>Scrambler>Shooter), so Rex's "always picks the style
  that beats yours" logic works unchanged.
- Bull/Thrower/Wall are captain-only, assigned to Capitol (Thrower/The
  Senator), Bascom (Wall/The Professor), and St. Louis (Bull/The
  Closer) — none of these three captains are built in `world.js` yet.
- This touches `roster.js` (every wrestler's `style` field), `moves.js`
  (the `ADV` export), existing captain flavor text, and any balance-sim
  assumptions about the old single-loop chart. Story and naming come
  first, per Tony's sequencing — implementation is a separate future
