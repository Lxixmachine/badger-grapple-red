import {loadState,saveState} from '../systems/save.js';import {FONT,uiBox,setVirtualHandler} from '../systems/ui.js';
...
export class IntroScene extends Phaser.Scene{
  ...
  draw(){this.children.removeAll();this.add.text(160,20,'BADGER GRAPPLE RED',{fontFamily:FONT,fontSize:16,color:'#f8f0d8',fontStyle:'bold',stroke:'#111',strokeThickness:4}).setOrigin(.5);this.add.text(160,44,'OPENING DAY',{fontFamily:FONT,fontSize:12,color:'#ffe28a',fontStyle:'bold',stroke:'#111',strokeThickness:2}).setOrigin(.5);this.add.ellipse(160,100,96,56,0x111015,.45);this.add.image(160,100,'portrait_badger').setScale(.56);uiBox(this,20,148,280,64);if(this.naming){this.add.text(32,154,'Choose your name:',{fontFamily:FONT,fontSize:12,color:'#111',fontStyle:'bold'});NAMES.forEach((n,i)=>this.add.text(42+(i%2)*118,173+(i>1?17:0),`${i===this.nameSel?'▶':' '} ${n}`,{fontFamily:FONT,fontSize:11,color:i===this.nameSel?'#b41820':'#111',fontStyle:i===this.nameSel?'bold':'normal'}));return;}const [who,txt]=PAGES[this.page];this.add.text(32,154,who,{fontFamily:FONT,fontSize:11,color:'#b41820',fontStyle:'bold'});this.add.text(32,168,txt,{fontFamily:FONT,fontSize:11,color:'#111',wordWrap:{width:250},lineSpacing:1});this.add.text(284,198,'A',{fontFamily:FONT,fontSize:12,color:'#555',fontStyle:'bold'});}
}
