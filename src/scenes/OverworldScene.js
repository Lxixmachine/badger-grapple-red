...
showObjectivePopup(title,body){const c=this.addUi(this.add.container(0,0).setScrollFactor(0).setDepth(1060));const g=this.add.graphics().setScrollFactor(0);g.fillStyle(0x000000,.35);g.fillRoundedRect(39,43,242,58,4);g.fillStyle(0xfff6dc,1);g.fillRoundedRect(36,40,242,58,4);g.lineStyle(2,0x111111,1);g.strokeRoundedRect(36,40,242,58,4);g.lineStyle(1,0xb41820,1);g.strokeRoundedRect(40,44,234,50,2);g.lineStyle(1,0xd6a336,.65);g.lineBetween(45,93,269,93);const t=this.add.text(157,47,title,{fontFamily:FONT,fontSize:12,color:'#b41820',fontStyle:'bold'}).setOrigin(.5).setScrollFactor(0);const b=this.add.text(157,68,body,{fontFamily:FONT,fontSize:11,color:'#111',fontStyle:'bold',align:'center',wordWrap:{width:216}}).setOrigin(.5).setScrollFactor(0);c.add([g,t,b]);this.tweens.add({targets:c,y:-8,alpha:0,delay:1250,duration:480,onComplete:()=>c.destroy(true)});}
 ...
 addNpc(x,y,frame=1,anim='npc-idle-down',dialogue='Keep working.',route=null,look=null){const sx=this.worldX(x),sy=this.actorWorldY(y),scale=this.actorScale();const requested=npcTextureKey(look),key=this.textures.exists(requested)?requested:'npc';const sh=this.addActor(this.add.ellipse(sx,sy-2,17,6,0x000000,.28).setDepth(sy-1).setScale(scale));const npc=this.addActor(this.add.sprite(sx,sy,key,frame).setOrigin(.5,1).setDepth(sy).setScale(scale));npc.setFlipX(false);npc.clearTint();npc.dialogue=dialogue;npc.home={x,y};npc.tile={x,y};this.npcList.push({npc,sh,t:0,route,i:0});return npc;}
 drawActors(){for(const obj of this.actors)obj.destroy();this.actors=[];this.npcList=[];const a=areaFor(this.area);const cap=a.captain;if(cap){this.addNpc(cap.x,cap.y,1,'npc-idle-down','Captain: Prove it on the mat.',null,'captain');}for(const tr of trainersInArea(this.area)){this.addNpc(tr.pos.x,tr.pos.y,DIRS[tr.facing]?.frame||1,'npc-idle-'+(tr.facing||'down'),tr.line||'Ready to wrestle.',null,tr.look||null);}for(const npc of layeredNpcs(this.area)){const g=npc.gate&&this.state.flags[npc.gate.flag]?npc.gate:null;this.addNpc(g?g.x:npc.x,g?g.y:npc.y,npc.frame,npc.anim,(g&&g.dialogue)||npc.dialogue,npc.route||null,npc.look||null);}if(this.area==='championship'){this.addNpc(TOURNAMENT.desk.x,TOURNAMENT.desk.y,7,'npc-idle-right','Official: The Big Ten Championship desk.',null,'official');}
}
