...
showObjectivePopup(title,body){const c=this.addUi(this.add.container(0,0).setScrollFactor(0).setDepth(1060));const g=this.add.graphics().setScrollFactor(0);g.fillStyle(0x000000,.35);g.fillRoundedRect(45,47,236,48,4);g.fillStyle(0xfff6dc,1);g.fillRoundedRect(42,44,236,48,4);g.lineStyle(2,0x111111,1);g.strokeRoundedRect(42,44,236,48,4);g.lineStyle(1,0xb41820,1);g.strokeRoundedRect(46,48,228,40,2);g.lineStyle(1,0xd6a336,.65);g.lineBetween(51,87,269,87);const t=this.add.text(160,51,title,{fontFamily:'monospace',fontSize:10,color:'#b41820',fontStyle:'bold'}).setOrigin(.5).setScrollFactor(0);const b=this.add.text(160,68,body,{fontFamily:'monospace',fontSize:8,color:'#111',align:'center',wordWrap:{width:210}}).setOrigin(.5).setScrollFactor(0);c.add([g,t,b]);this.tweens.add({targets:c,y:-8,alpha:0,delay:1250,duration:480,onComplete:()=>c.destroy(true)});}
 ...
 drawActors(){for(const obj of this.actors)obj.destroy();this.actors=[];this.npcList=[];const a=areaFor(this.area);const cap=a.captain;if(cap){this.addNpc(cap.x,cap.y,1,'npc-idle-down','Captain: Prove it on the mat.',null,'gold');}for(const tr of trainersInArea(this.area)){this.addNpc(tr.pos.x,tr.pos.y,DIRS[tr.facing]?.frame||1,'npc-idle-'+(tr.facing||'down'),tr.line||'Ready to wrestle.',null,tr.look||null);}for(const npc of layeredNpcs(this.area)){this.addNpc(npc.x,npc.y,npc.frame,npc.anim,npc.dialogue,npc.route||null,npc.look||null);}if(this.area==='lakeshore'){
  this.addNpc(18,7,4,'npc-idle-left','Jogger: Every morning along Mendota. Watch the shoreline crew - they love an ambush.',[[18,7],[19,7],[19,8],[18,8]],'red');
}if(this.area==='river'){
  this.addNpc(3,4,1,'npc-idle-down','Camper: The Funk Doctor holds court by the fire circle. People hike all the way out here just to get humbled.',null,'gray');
}if(this.area==='downtown'){
  this.addNpc(7,8,1,'npc-idle-down','Shopper: Every storefront on State Street is somebody\'s game-day tradition.',[[7,8],[8,8],[8,9],[7,9]],'purple');
  this.addNpc(25,6,10,'npc-idle-up','Fan: From the Kohl Center marquee you can see the Capitol dome. Two ends of the same street - the season runs between them.',null,'green');
 ...
 updateMarker(){const kind=this.kindHere();this.marker.setVisible(['EXIT','R','S','C','g','M','N','STATUE','SCOUT_NPC','TRAINER','TOURNEY','SAVE_NPC','BATTLE_NPC','STUDY_NPC','HIDDEN_TAPE','HIDDEN_FILM','HIDDEN_DRINK','DOOR','NATIONALS','CAPITOL','SIGN','WEIGHT_ROOM','LOCKER_ROOM','EQUIP_ROOM','COACH_OFFICE','RECEPTION','MEETING_ROOM'].includes(kind)&&!this.messageOpen);if(this.marker.visible){this.marker.setPosition(this.player.x,this.player.y-35+Math.sin((this.time.now||0)/180)*1.2);this.marker.setAlpha(.86+Math.sin((this.time.now||0)/160)*.12);}}
