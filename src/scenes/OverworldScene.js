...
interact(){this.unlockSfx();if(this.messageOpen){this.clearMessage();return;}const kind=this.kindHere();this.faceNpcAtFront();this.time.delayedCall(60,()=>{});this.playSfx('talk');if(kind==='R')return this.recover();if(kind==='S')return this.scene.launch('MenuScene',{parent:this,tab:'shop'});if(kind==='C')return this.startGymBattle(areaFor(this.area).captain);if(kind==='TOURNEY')return this.tournamentDesk();if(kind==='g')return this.startScout();if(kind==='M')return this.startBattle('drillpartner',5,'spar');if(kind==='N')return this.coachObjective();if(kind==='STATUE')return this.showMessage(this.statueLore());if(kind==='SCOUT_NPC')return this.showMessage('Scout: An open mat gives a scout report first. Wear a prospect down before offering a Wisconsin singlet.');if(kind==='TRAINER'){const tr=this.trainerNearby();if(tr)return this.startTrainerBattle(tr);}if(kind==='SAVE_NPC')return this.showMessage('Student Manager: Use START for the menu or SAVE for quick save. Continue keeps your roster and location.');if(kind==='BATTLE_NPC')return this.showMessage('Wrestler: Sparring is inside on the mat. Open-mat prospects can join your travel lineup or team locker.');if(kind==='STUDY_NPC')return this.showMessage('Tutor: Film Study improves your next three recruiting attempts.');if(kind==='HIDDEN_TAPE')return this.hiddenItem('quadTape','athleticTape','Found Athletic Tape hidden near the benches.');if(kind==='HIDDEN_FILM')return this.hiddenItem('studyFilm','filmStudy','Found Film Study notes on the desk.');if(kind==='HIDDEN_DRINK')return this.hiddenItem('quadDrink','sportsDrink','Found a Sports Drink under the bench.');if(kind==='HIDDEN_SHORE')return this.hiddenItem('pierDrink','sportsDrink','Found a Sports Drink left on the pier planks.');if(kind==='HIDDEN_EMBER')return this.hiddenItem('emberSinglet','practiceSinglet','Found a Practice Singlet tucked under a fire-circle stone.');if(kind==='TROPHY')return this.showMessage(this.trophyLore());if(kind==='DOOR')return this.showMessage('Memorial Library entrance. Step forward to enter.');if(kind==='NATIONALS')return this.showMessage('NATIONALS ACCESS - Season Two. Qualification door locked. The next campaign starts beyond it.');if(kind==='CAPITOL')return this.showMessage('The Capitol dome rises where State Street meets the Square. Every championship run in this state ends with a march up this street.');if(kind==='SIGN'){const f=this.frontTile();return this.showMessage(signText(this.area,f.x,f.y)||signText(this.area,this.tilePos.x,this.tilePos.y)||'A weathered sign.');}if(kind==='WEIGHT_ROOM')return this.scene.launch('MenuScene',{parent:this,tab:'practice'});if(kind==='BUS_STOP')return this.state.keyItems?.busPass?this.scene.launch('MenuScene',{parent:this,tab:'travel'}):this.showMessage("The bus board is active, but travel requires the Bus Pass from The Senator's staff.");if(kind==='LOCKER_ROOM'||kind==='TRAINER_LOCKER'){if(!this.state.flags?.lockerUnlocked)return this.showMessage('The Team Locker is not assigned yet. Coach unlocks it with the Roster Book.');return this.scene.launch('MenuScene',{parent:this,tab:'locker'});}if(kind==='EQUIP_ROOM')return this.showMessage('Equipment Room: tape, drinks, Film Study, and singlets support recruiting trips.');if(kind==='COACH_OFFICE')return this.showMessage('Coach Office: Review objectives, recruit with purpose, and return after your first win.');if(kind==='RECEPTION')return this.showMessage('Reception: Visitors sign in here before walking the room.');if(kind==='MEETING_ROOM')return this.showMessage('Team Room: Film Study raises recruiting confidence.');this.showMessage(this.promptFor(kind)||'Nothing unusual here.');}
 trophyLore(){
