...
export class BattleScene extends Phaser.Scene{
  ...
  drawCommand(lr){this.drawTextBox(3,132,114,35);this.add.text(9,141,'What will',{fontFamily:'monospace',fontSize:8,color:'#111',fontStyle:'bold'});this.add.text(9,154,`${lr.name.split(' ')[0]} do?`,{fontFamily:'monospace',fontSize:8,color:'#111',fontStyle:'bold'});this.drawCommandBox(119,132,118,35);COMMANDS.forEach((t,i)=>this.add.text(128+(i%2)*56,141+(i>1?13:0),`${i===this.sel?'\u25b6':' '}${t}`,{fontFamily:'monospace',fontSize:8,color:i===this.sel?'#8a1720':'#111',fontStyle:'bold'}));this.drawBattleLog(6,115,2);}
  ...
  drawMoveTag(mv){const g=this.add.graphics();g.fillStyle(0x111015,.88);g.fillRoundedRect(129,115,106,13,2);g.lineStyle(1,0xd6a336,.9);g.strokeRoundedRect(129,115,106,13,2);this.add.text(134,119,`${mv.style} P${mv.power} ACC${Math.round(mv.acc*100)} EP${mv.gas}`,{fontFamily:'monospace',fontSize:6,color:'#f8f0d8',fontStyle:'bold'});}
  drawArenaBackdrop(){this.add.image(0,0,'battle_arena').setOrigin(0);const g=this.add.graphics();g.fillStyle(0xffffff,.05);g.fillRect(0,0,240,66);g.fillStyle(0x000000,.10);g.fillRect(0,126,240,44);g.lineStyle(2,0x7b1d2a,.9);g.strokeEllipse(120,99,174,52);g.lineStyle(1,0xd6a336,.55);g.strokeEllipse(120,99,143,41);g.lineStyle(1,0xffffff,.32);g.strokeEllipse(120,99,102,29);}
