...
export function chooseStarter(id,options={}){const existing=loadState();const s=normalizeState({...defaultState(),...existing});s.party=[makeMon(id,6)];s.box=[];s.active=0;s.dex.seen[id]=true;s.dex.caught[id]=true;s.flags.introDone=true;s.flags.personaChosen=true;if(options.story){s.flags.coachIntro=true;s.flags.assignment=false;s.objective={id:'opening_persona',stage:1,complete:false,log:['Report to Coach on the mat','Meet the Head Coach']};s.area=options.area||existing.area||'wrestlingroom';s.pos=options.pos||existing.pos||{x:10,y:6};s.message=options.message||'Coach: That is your mat persona. Talk to me when you are ready for the first assignment.';}else{s.flags.coachIntro=false;s.flags.assignment=false;s.objective={id:'meet_coach',stage:1,complete:false,log:['Meet the Head Coach']};s.area='fieldhouse';s.pos={x:10,y:10};s.message='Coach is in the wrestling room. The captain is waiting at the trophy threshold.';}saveState(s);return s;}
