...
export function chooseStarter(id){const existing=loadState();const s=normalizeState({...defaultState(),...existing});s.party=[makeMon(id,6)];s.box=[];s.active=0;s.dex.seen[id]=true;s.dex.caught[id]=true;s.flags.introDone=true;s.flags.coachIntro=false;s.flags.assignment=false;s.objective={id:'meet_coach',stage:1,complete:false,log:['Meet the Head Coach']};s.area='fieldhouse';s.pos={x:7,y:12};s.message='Coach is in the wrestling room. The captain is waiting at the trophy threshold.';saveState(s);return s;}
